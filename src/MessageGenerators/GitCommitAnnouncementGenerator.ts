...
const PROMPT = ChatPromptTemplate.fromPromptMessages([
    ...
    AIMessagePromptTemplate.fromTemplate('Ich habe ein Update! Neu wird eine Erwähnung meines Usernamens nicht mehr entfernt, sondern ersetzt. Das sorgt für besseres Reagieren auf Nachrichten.'),
    HumanMessagePromptTemplate.fromTemplate(`Apply WitReplyStrategy also to replies

In addition to mentions.`),
    AIMessagePromptTemplate.fromTemplate('Guten Tag, ich habe ein neues Feature. Neu wird die Wit-Antwort-Strategie auch bei Antworten statt nur bei Erwähnungen angewendet.'),
    HumanMessagePromptTemplate.fromTemplate(`Use only largest GPT-3 model

In preparation for OpenAI's price cuts`),
    AIMessagePromptTemplate.fromTemplate('Ich verkünde: Neu wird immer das grösste GPT-3-Modell genutzt, da OpenAI die Preise senken wird.'),
...
Also refactor commands to an enum.`),
    AIMessagePromptTemplate.fromTemplate('Grüezi, ich habe eine Ankündigung. Neu nutze ich eine Kommentar-Antwort-Strategie. Diese sorgt dafür, dass ich eine Nachricht kommentiere, wenn jemand (ausschliesslich) mit meinem Username (inkl. @) antwortet. Zudem nutze ich jetzt ein Enum für meine Kommandos.'),
    HumanMessagePromptTemplate.fromTemplate('Set @types/node to LTS node version'),
    AIMessagePromptTemplate.fromTemplate('Ganz neu: Ich nutze jetzt – für Verbesserung meiner Arbeitsqualität und weiteres – die Versionsnummer von Node LTS für meine @types/node-Bibliothek. Genial!'),
