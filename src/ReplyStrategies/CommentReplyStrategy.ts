...
export class CommentReplyStrategy extends AllowlistedReplyStrategy {
  ...
  constructor(
    ...
    super(config);
    this.onlyUsernameRegExp = new RegExp(`^\\w*@${this.config.primaryBot.username}\\w*$`, 'is');
  }

  willHandleAllowlisted(message: MessageModel): boolean {
    return message.imageFileId === null && this.onlyUsernameRegExp.test(message.text);
  }

  async handle(message: TelegramMessageWithReplyTo): Promise<void> {
    void this.telegram.sendTyping(message.chatId);
    const streamSession = this.telegram.createModelTextSession(
      message.chatId,
      message.telegramMessageId,
    );
    const reply = await this.command.execute(Commands.Comment, message, streamSession);
